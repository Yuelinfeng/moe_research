...
`Qwen/Qwen1.5-MoE-A2.7B-Chat` through `https://hf-mirror.com`. It records
environment checks, `nvidia-smi`, latency, token throughput, and peak CUDA
memory. Set `BASELINE_MODE=synthetic` to run the network-independent Tiny-MoE
fallback.
...
powershell -ExecutionPolicy Bypass -File scripts/local_push_run_fetch.ps1 `
  -Config configs/remote.local.json `
  -RunLabel synthetic_moe_baseline `
  -RunCommand 'BASELINE_MODE=synthetic MAX_NEW_TOKENS=32 bash experiments/run_baseline.sh --output-dir "$RUN_DIR"'
```
